...
8. **`object`** (`std::string`) — **Имя цели** для радарных сообщений (`TLL`, `TTM`, `RATTRM`, `$PFEC,TLL`).
9. **`lat`** (`double`) — **Широта**. Географическая широта в десятичных градусах (WGS-84), нормализованная с учетом знака полушария. При поступлении плоских метров из проприетарных строк (*Fugro/Trimble*), они автоматически пересчитываются сюда обратным геодезическим движком.
10. **`lon`** (`double`) — **Долгота**. Географическая долгота в десятичных градусах (WGS-84).
...
13. **`ais_msg_type_descr`** (`std::string`) — **Расшифрованный тип сообщения AIS** согласно ITU-R M.1371.
...
*Примечание: При разборе статических рейсовых данных (AIS тип 5) структура дополнительно дописывает параметры текстового рейса: `ais_ship_name` (Название судна), `ais_call_sign` (Позывной) и `ais_dest_port` (Пункт назначения).*
